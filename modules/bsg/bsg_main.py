...
if __name__ == '__main__':
    ...
    parser.add_argument('--multi_weights', default='0.9,0.1')
    ...
    # TODO backward compatability only
    if not hasattr(vocab, 'metadata_start_id'):
        vocab.metadata_start_id = vocab.section_start_vocab_id

    print('Loaded vocabulary of size={}...'.format(vocab.metadata_start_id))

    print('Collecting metadata information')
    sec_id_range = np.arange(vocab.metadata_start_id, vocab.size())

    # Indices in ids held by section and category ids, respectively
    # I.e. for example ids array [header=DATE, today, header=SURGICALPROCEDURE, none]
    # sec_ids = [1, 3]
    ...
    # then sec_ids = [-1, 2, 2, 3, 3]
    sec_ids = enumerate_metadata_ids(ids, sec_pos_idxs)
    print('Snippet from beginning of data...')
    for ct, (sid, tid) in enumerate(zip(sec_ids, ids)):
        print('\t', vocab.get_tokens([sid, tid]))
        if ct >= 10:
            break
    ...
    batcher = BSGBatchLoader(len(ids), full_pos_idxs, batch_size=args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i in tqdm(range(num_batches)):
            ...
            batch_ids = batcher.next(ids, sec_ids, vocab, args.window)
