...
from bsg_encoder import BSGEncoder
from compute_utils import compute_kl, mask_2D


class BSG(nn.Module):
    """
    Main Bayesian Skip-Gram model as described in the paper.

    It returns the KL-Divergence Hinge Loss and the reconstruction loss

    - the KL-Divergence Hinge Loss between a latent contextualized density and the individual context words within it.
    The hinge loss is a margin between the KL-Divergence between an actual word in the context sequence and a single
    negatively sampled word

    - The reconstruction loss measures the divergence between the center words contextualized represenation and its
    prior density
    """
    ...
    def forward(self, token_ids, sec_ids, context_ids, neg_context_ids, num_contexts):
        """
        :param center_ids: batch_size
        :param sec_ids: batch_size
