        ...
        for pattern in patterns:
            if type(pattern) == tuple:
                ...
                if re.match(r'^(.*?)' + pattern, section, flags=re.IGNORECASE):
                    category = _to_snake(category) if canonical in USE_TYPE_PREFIX else 'CANONICAL'
                    return (_to_snake(category) + '->' + canonical).upper()
...
if __name__ == '__main__':
    df = pd.read_csv('data/mimic/section_freq.csv')
    raw_sections = df['section'].tolist()
    canonical_sections = list(map(get_canonical_section, raw_sections))
    df['standard'] = canonical_sections
    print(len(set(canonical_sections)))
    df.to_csv('data/mimic/section_canonical.csv', index=False)
