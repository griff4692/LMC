...
def preprocess_mimic(input):
    ...
    for tok_idx, toks in enumerate(sectioned_text):
        ...
        if is_header and is_actual_header(toks):
            ...
            full_tokenized_text += tokenize_str(clean_text(toks), stopwords=stopwords)
    ...
    for doc_idx, doc in enumerate(parsed_docs):
        for token in doc.split():
            ...
            if 'header=' not in token and token not in SPECIAL_TOKS:
                token_cts['__ALL__'] += 1
    debug_str = '_mini' if args.debug else ''
    out_fn = 'data/mimic/NOTEEVENTS_'
    out_tok_fn = out_fn + 'tokenized{}.json'.format(debug_str)
    out_counts_fn = out_fn + 'token_counts{}.json'.format(debug_str)
