...
def compute_att(h, mask, att_linear):
    """
    :param h: FloatTensor of batch_size x context len x dim
    :param mask: BoolTensor of batch_size x context len x dim.  mask=True for padded tokens.
    :param att_linear: a weight projection from dim to 1
    :return:
    """
    att_scores = att_linear(h).squeeze(-1)
    # Mask padding with large negative number rather than infinity for numerical stability
...
def mask_2D(target_size, num_contexts):
    """
    :param target_size: torch.Size object representing target size of mask: batch_size x max context len.
    :param num_contexts: Iterable of batch_size representing non-padding lengths of every sequence
    Dim[0] of target_size must = Dim[0] of num_contexts
    :return:
    """
