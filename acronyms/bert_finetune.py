...
from pre_trained_bert import get_pretrained_bert
...
def run_test_epoch_bert(model, test_batcher, bert_tokenizer, bert_config, sf_tokenized_lf_map, loss_func):
    model.eval()
    test_batcher.reset(shuffle=False)
    test_epoch_loss, test_examples, test_correct = 0.0, 0, 0

    for _ in tqdm(range(test_batcher.num_batches())):
        batch_input, num_outputs = test_batcher.bert_next(bert_tokenizer, sf_tokenized_lf_map)
        batch_input = list(map(lambda x: x.long().clamp_min_(0).to('cuda'), batch_input))
        with torch.no_grad():
            scores, target = model(*batch_input + [num_outputs])
        num_correct = len(np.where(tensor_to_np(torch.argmax(scores, 1)) == tensor_to_np(target))[0])
        num_examples = len(num_outputs)
...
def bert_finetune(args, loader):
    ...
    bert_pretrained_model, bert_tokenizer, bert_config = get_pretrained_bert()
...
#    bert_model = bert_model.to('cuda')
    trainable_params = list(filter(lambda x: x.requires_grad, bert_pretrained_model.parameters()))
    ...
    bert_model = BertAcronymExpander(bert_pretrained_model,cls_only=False)
    bert_model = bert_model.to('cuda')
    tst_loss = run_test_epoch_bert(bert_model, test_batcher, bert_tokenizer, bert_config, sf_tokenized_lf_map, loss_func)
