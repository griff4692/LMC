...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Script to compare errors from two different models')

    # Functional Arguments
    parser.add_argument('--e1', default='bsg_mimic')
    parser.add_argument('--e2', default='lmc_mimic')

    """
    e1 and e2 must be the paths to models which have already undergone evaluation (see evaluate.py)
    
    Computes intersection and union of errors and serializes to a joint results directory.
    """
    args = parser.parse_args()
    weights_dir = os.path.join(home_dir, 'weights', 'acronyms')

    e1_fn = os.path.join(weights_dir, args.e1, 'results', 'error_tracker.json')
    e2_fn = os.path.join(weights_dir, args.e2, 'results', 'error_tracker.json')
    ...
    print('Saving comparison results to {}'.format(out_fn))
