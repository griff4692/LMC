        ...
        for sentence in doc_sentences:
            ...
            if len(tokens) > 1 and len(list(filter(lambda x: 'header' not in x, tokens))) > 0:
                ...
                if len(sentences) % 1000000 == 0:
                    with open('data/chunks/sentences{}{}_{}.txt'.format(debug_str, phrase_str, chunk), 'w') as fd:
                        fd.write('\n'.join(list(sentences)))
                    sentences = []
                    chunk += 1
    if len(sentences) > 0:
        with open('data/sentences{}{}_{}.txt'.format(debug_str, phrase_str, chunk), 'w') as fd:
            fd.write('\n'.join(list(sentences)))
        sentences = []
        chunk += 1
    with open('data/sentences{}{}.txt'.format(debug_str, phrase_str), 'a') as out_fd:
        for cidx in range(chunk):
            with open('data/chunks/sentences{}{}_{}.txt'.format(debug_str, phrase_str, cidx), 'r') as in_fd:
